...
`&#38;` is a numeric HTML entity (the code for `&`) which, before `_strip_html` was extended to `nom_historique` (see section 1), was only fixed by `ftfy`; it is now decoded upstream by BeautifulSoup as well, and `ftfy` remains responsible for pure mojibake (e.g. `Ã©` → `é`).
